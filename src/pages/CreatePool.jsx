...
const CreatePoolSection = () => {
  ...
  const viewPrevEvent = async () => {
    ...
    try {
      ...
      console.log("Decoded PoolCreated Events:");
      events.forEach((evt, index) => {
        console.log(`Event ${index + 1}:`);
        console.log("Pool ID:", evt.args.poolId?.toString());
        console.log("Name:", evt.args.name);
        console.log(
          "End Time:",
          new Date(Number(evt.args.endTime) * 1000).toLocaleString()
        );
        console.log("Borrower:", evt.args.borrower);
        console.log("Block Number:", evt.blockNumber);
        console.log("---------------------------");
      });
    } catch (error) {
      console.log("🚀 ~ viewPrevEvent ~ error:", error);
    }
  };
